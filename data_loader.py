...
from config import datapath, decimateddatapath, DAS_FILE_FORMAT, DECIM_FILE_FORMAT, DATE_FORMAT, dx, length_sw
...
class DataLoader:
    ...
    def scan_data(self, start_date: str, end_date: str, start_hour: int, end_hour: int):
        ...
        for date in date_range:
            ...
            if not file_day_dir.exists():
                raise FileNotFoundError(f"Repertory not found for date {date} and path {file_day_dir}")
    ...
    def get_next_data(self):
        ...
        if len(corresponding_decimated_files) > 0:
            ...
            create_npz_data(fp, decimated_day_rep, return_full=False)
